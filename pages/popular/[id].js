  ...
  return (
    <div className="mtop grid md:grid-cols-2 wrapper">
      ...
      <div className="p-5">
        ...
        <h2 className="my-5">gender:${data.cat}</h2>
...
export async function getStaticPaths() {
  const { popular } = await import("../../pages/data/data.json");
  const allpaths = popular.map((path) => {
    return {
      params: {
        id: path.slug,
      },
...
export async function getStaticProps(context) {
  ...
  const { popular } = await import("../../pages/data/data.json");
  const eventsData = popular.find((ev) => id === ev.slug);
  return {
