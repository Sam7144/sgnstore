...
function singleProduct({ data }) {
  ...
  return (
    <div className="mtop grid md:grid-cols-2 wrapper">
      <Image src={data.image} alt={data.name} width={500} height={500} className="imgim"></Image>
      <div className="p-5">
        <h1 className="my-3">origin:{data.origin}</h1>
        <h1 className="my-5">Gender:{data.gender}</h1>
...
export async function getStaticPaths() {
  const { morgan } = await import("../../pages/data/data.json");
  const allpaths = morgan.map((path) => {
    return {
      params: {
        id: path.name,
      },
...
export async function getStaticProps(context) {
  ...
  console.log(context);
  const { morgan } = await import("../../pages/data/data.json");
  const eventsData = morgan.find((ev) => id === ev.name);
