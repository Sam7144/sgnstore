  ...
  return (
    <div className="mtop grid md:grid-cols-2 wrapper">
      ...
      <div className="p-5">
        ...
        <h1 className="my-5">Gender:{data.cat}</h1>
...
export async function getStaticPaths() {
  const { basket } = await import("../../pages/data/data.json");
  const allpaths = basket.map((path) => {
    return {
      params: {
        id: path.slug,
      },
...
export async function getStaticProps(context) {
  ...
  const { basket } = await import("../../pages/data/data.json");
  const eventsData = basket.find((ev) => id === ev.slug);
  return {
